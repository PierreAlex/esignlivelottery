...
public class DrawTest extends TestCase {
	...
	public void testBigDraw(){
		...
		lottery.draw();
		assertEquals("First prize pool is incorect",225.0, lottery.getResult().getFirstPrize() );
		assertEquals("Second prize pool is incorect",45.0, lottery.getResult().getSecondPrize() );
		assertEquals("Third prize pool is incorect",30.0, lottery.getResult().getThirdPrize() );
